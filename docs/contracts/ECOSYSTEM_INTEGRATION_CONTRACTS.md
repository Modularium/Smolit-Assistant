...
| 4 | Smolit-Assistant ↔ OceanData | Assistant → OceanData: `decide_access` für externen Provider-Export, `query_context` / `list_available_contexts` / `fetch_context_summary` als read-only Kontext-Pfad. | Smolit-Assistant [`ADR-0004`](../adr/ADR-0004-oceandata-data-layer-integration.md) (Boundary-Rahmen) + [`ADR-0006`](../adr/ADR-0006-oceandata-context-provider-spi.md) (Context-Provider-SPI seit PR 48); OceanData [`docs/integrations/smolit_assistant.md`](https://github.com/EcoSphereNetwork/OceanData/blob/main/docs/integrations/smolit_assistant.md) | proposed / docs-only (beidseitig); SPI-Form via ADR-0006 fixiert | **none in Smolit-Assistant** (kein Provider-Kind, kein IPC, keine ContextProvider-Trait-Implementation); OceanData-Seite hat ausführbare Test-Vektoren | data / privacy | read-only lokale Kontext-Summary mit `redaction = local_only`, `allow_sensitive = false`, `allow_external_forwarding = false`; lokal-first (Unix-Socket / Loopback); kein Cloud-Default; **niemals** in `text_provider_chain` aufgenommen | Sensitivity-/Provenance-Schema (ADR-0004 FA-4); Privacy-/Redaction-Gate (ADR-0004 FA-5 + ADR-0006 OC-2) **vor** externer Weitergabe; bounded `max_items` / `max_summary_chars`; Audit Correlation ID **Implementation** für Aktionskontext; Honoring von `AccessDecision.effect` (`deny` / `require_approval` / `allow_local_only` / …) | ADR-0006 fixiert Achsen-Form, Object-Model, 13 Failure-Modes. Implementation bleibt aufgeschoben hinter ADR-0006 §17 OC-1…OC-7 |
| 5 | ABrain ↔ OceanData | ABrain → OceanData: `decide_access` vor Tool-/Provider-Use; `compute_jobs` für anonymisierten Export. | OceanData [`docs/integrations/abrain.md`](https://github.com/EcoSphereNetwork/OceanData/blob/main/docs/integrations/abrain.md) ist **kanonisch**; ABrain referenziert OceanData heute nur als negative Boundary in `SMOLIT_ASSISTANT_NATIVE_API.md` §2 + §"What this contract is not" | asymmetric docs-only — kanonisch nur OceanData-seitig | none **durch Smolit-Assistant**; eine direkte ABrain ↔ OceanData-Linie würde Smolit-Assistant nicht kreuzen | data exfiltration / overbroad context, falls ABrain unrestrictierten OceanData-Zugriff bekäme | redacted summaries only (gespiegelt durch Smolit-Assistant ADR-0004 D8, ADR-0003 §"OceanData-Boundary" und ADR-0006 §13: ABrain bekommt OceanData-Kontext **nur** bounded/redacted **über** Smolit-Assistant, nie direkt) | purpose-bound access; **kein** unrestrictierter Data-Lake-Zugriff; Privacy-/Redaction-Gate (ADR-0006 OC-2 / ADR-0004 FA-5); Honoring `AccessDecision` exakt | **PR 44/PR 48 erzeugen keine neue ABrain-Berechtigung.** Naming-Drift in der ABrain-Seite (Spiegel fehlt) ist bekannt; eine spätere Spiegel-PR liegt im ABrain-Repo, nicht hier |
...
- [`docs/adr/ADR-0005-adminbot-safety-boundary.md`](../adr/ADR-0005-adminbot-safety-boundary.md) —
...
- [`docs/adr/ADR-0006-oceandata-context-provider-spi.md`](../adr/ADR-0006-oceandata-context-provider-spi.md) —
  OceanData Context Provider SPI (Proposed, PR 48,
  Docs/ADR-only; Achsen-Form parallel zu Text/STT/TTS,
  Kandidaten-Kinds `local_static_context` / `oceandata_context`,
  Object-Model für ProviderConfig + Request/Response, 13 benannte
  Failure-Modes; FA-2-Lesart aus ADR-0004 §11).
...
- **Smolit-Assistant ↔ AdminBot Safety Boundary ADR — gelandet
  in PR 45** ([`ADR-0005`](../adr/ADR-0005-adminbot-safety-boundary.md),
  Proposed, Docs/ADR-only). Schließt den Designrahmen für die

...
| 46 | E | **Audit Correlation ID + Capability Vocabulary** (Vorschlag): cross-repo Korrelations-ID-Spec (Smolit-Assistant Audit ↔ ABrain-Adapter ↔ AdminBot-IPC) plus gemeinsames Capability-Vokabular (`status_read`, `service_status`, `action_intent`, `context_summary`, …). Voraussetzung für `correlation_id_required = true` in ADR-0005. **Keine** IPC-Erweiterung im Code. |
